...
option_list = list(
  ...
  make_option(c("-s", "--samplefile"), type="character", default="data/samples.txt", help="sample files used to get conditions for DESEq2 model fit", metavar="character"),
...
if (length(conditions) == 1){
  ...
  samplefile$conditions = paste(samplefile[,conditions[1]],samplefile[,conditions[2]],sep = "")
  condition <- factor(samplefile[,"conditions"])
} else if (length(conditions > 2)){
